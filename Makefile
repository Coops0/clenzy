...
setup:
	...
	rustup target add --toolchain nightly $(X86_64_PC_WINDOWS_GNU)
	rustup target add --toolchain nightly $(X86_64_UNKNOWN_LINUX_GNU)
	rustup target add --toolchain nightly $(AARCH64_UNKNOWN_LINUX_GNU)
...
x86_64-pc-windows-gnu: $(DIST_DIR)
	cargo +nightly build --release --target $(X86_64_PC_WINDOWS_GNU)
...
x86_64-unknown-linux-gnu: $(DIST_DIR)
	cargo +nightly build --release --target $(X86_64_UNKNOWN_LINUX_GNU)
...
aarch64-unknown-linux-gnu: $(DIST_DIR)
	cargo +nightly build --release --target $(AARCH64_UNKNOWN_LINUX_GNU)
